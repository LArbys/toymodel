...
def main(MPID_FILE,OUT_DIR, CFG):
    ...
    tree=f.Get("nue_ana_precut_tree")

    from toynet import toy_perceptron
    input_data = tf.placeholder("float", [None, 5])
    p_net = toy_perceptron.multilayer_perceptron(input_data, 64)

    NUM = str(os.path.basename(MPID_FILE).split(".")[0].split("_")[-1])
    FOUT = os.path.join(OUT_DIR,"perceptron_out_%s.root" % NUM)
    ...
    for entry in tree:
        ...
        for plane in xrange(3):
            ...
            '''
            if(not inter_type): 
                print ('fuck',inter_type)
                print (entry.run, entry.subrun, entry.event)
            '''
            #print inter_type
